...
export const generateTypographyCSSVars = (): Record<string, string> => {
  ...
  Object.keys(TYPOGRAPHY_TOKENS).forEach((variant) => {
    Object.keys(TYPOGRAPHY_TOKENS[variant as TypographyVariant]).forEach(
      (device) => {
        Object.keys(
          TYPOGRAPHY_TOKENS[variant as TypographyVariant][device as DeviceType]
        ).forEach((language) => {
          const style =
            TYPOGRAPHY_TOKENS[variant as TypographyVariant][
